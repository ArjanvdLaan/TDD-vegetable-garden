...
const getRevenueForCrop = (element, factors) => {
    if (factors == null) {
        ...
        return element.salePrice * getYieldForCrop(element, factors) 
    };
};
...
const getYieldForPlant = (plant, factors) => {
    ...
    switch (factors.sun ) {
        case "low":
            factorSun += (plant.factor.sun.low + 100)/ 100;
            break;
        case "medium":
            factorSun += (plant.factor.sun.medium + 100)/ 100;
            break;
        case "high":
            factorSun += (plant.factor.sun.high + 100)/ 100;
            break;
        default: 
            factorSun = 0;
    }
    switch (factors.wind) {
        case "low":
            factorWind += (plant.factor.wind.low + 100)/100;
            break;
        case "medium":
            factorWind += (plant.factor.wind.medium + 100)/100;
            break;
        case "high":
            factorWind += (plant.factor.wind.high + 100)/100;
            break;
        default: 
            factorWind = 0;
    }
...
const getTotalYield = function (plants, factors) {
    if (factors == null) {
        ...
        let totalY = 0;
        plants.forEach(plant => {
        totalY += getYieldForCrop(plant, factors);
        });
        return totalY;
    }
}
